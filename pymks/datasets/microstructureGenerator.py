...
class MicrostructureGenerator(object):
    def __init__(self, n_samples, size, n_phases, grain_size):
    ...
    def generate(self):
        ...
        gaussian_filter = self._make_filter(size)
        gaussian_filter = np.tile(gaussian_filter, (n_samples,) +
                                  tuple(np.ones(len(gaussian_filter.shape),
                                                dtype=int)))
        X_blur = self._convolve_fft(X, gaussian_filter,
                                    axes=np.arange(len(X[0].shape)) + 1)
        return self._assign_phases(X_blur, n_phases)
