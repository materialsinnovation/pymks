...
>>> strain, _, _ = solve(X, elastic_modulus=(1.0, 10.0), poissons_ratio=(0., 0.), dx=1., macro_strain=1.)
...
def solve(x_data, elastic_modulus, poissons_ratio, dx, macro_strain=1.):
    sim = ElasticFESimulation(elastic_modulus=elastic_modulus, poissons_ratio=poissons_ratio)
    sim.run(x_data, macro_strain, dx)
    return (sim.strain, sim.displacement, sim.stress)
...
def _convert_properties(dim, elastic_modulus, poissons_ratio):
    ...
    >>> assert(np.allclose(
    ...     _convert_properties(dim=2, elastic_modulus=(1., 2.), poissons_ratio=(1., 1.)),
    ...
    ...                  [2, 1, 0, 0],
    ...                  [1, 0, 2, 2]]])
    >>> X2D_property = _convert_properties(dim=2, elastic_modulus=(1., 2., 3.), poissons_ratio=(1., 1., 1.))[X2D]
    ...
    ...                 [[1, 1],
    ...                  [0, 1]]])
    >>> X3D_property = _convert_properties(dim=2, elastic_modulus=(1., 2.), poissons_ratio=(1., 1.))[X3D]
...
class ElasticFESimulation(object):
    ...
    >>> macro_strain = 0.1
    >>> sim = ElasticFESimulation((10.0,1.0), (0.3,0.3))
    >>> sim.run(X, macro_strain=macro_strain)
    ...
    def __init__(self, elastic_modulus, poissons_ratio):
    ...
    def run(self, X, macro_strain=1., dx=1.):
        """
        Run the simulation.
        ...
        X_property = pipe(
            X,
            do(_check(len(self.elastic_modulus))),
            lambda x: _convert_properties(len(x.shape) - 1, self.elastic_modulus, self.poissons_ratio)[x]
        )
        strain = []
        displacement = []
        stress = []
        for x in X_property:
            strain_, displacement_, stress_ = self._solve(x, macro_strain=macro_strain, dx=dx)
    ...
    def _get_material(self, property_array, domain, dx):
        ...
        def _material_func_(ts, coors, mode=None, **kwargs):
            if mode == 'qp':
                ...
                ijk = np.floor((coors - min_xyz[None]) / dx,
    ...
    def _get_linear_combinationBCs(self, domain, macro_strain):
        ...
        def shift_(ts, coors, region):
            return np.ones_like(coors[:, 0]) * \
                macro_strain * (max_xyz[0] - min_xyz[0])
    ...
    def _solve(self, property_array, macro_strain, dx):
        """
        ...
        m = self._get_material(property_array, domain, dx)
