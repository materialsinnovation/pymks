...
import numpy as np
from fmks.bases import legendre as leg
...
def polyval(x_data):
    ...
    """
    x_data = 4 * x_data - 1
    polys = np.array((np.ones_like(x_data), x_data,
                      (3. * x_data**2 - 1.) / 2.))
...
def test_1():
    ...
    domain = [0., 0.5]
    chunks = (1,)
    assert(np.allclose(leg.legendre_basis(data,
                                          n_state,
                                          domain,
                                          chunks)[0].compute(),
                       polyval(data)))
...
def test_2():
    ...
    f_test = np.array([[[[4.50000000 + 0.j, - 0.79735949 + 0.],
